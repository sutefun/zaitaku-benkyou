                ...
                for j, i in product(range(conv_h), range(conv_w)):
                    ...
                    for ars in self.aspect_ratios:
                        ...
                        for scale in self.scales:
                            for ar in ars:
                                # yolact plus  'preapply_sqrt': False,
                                if not cfg.backbone.preapply_sqrt:
                                    ar = sqrt(ar)

                                # yolact plus base  'use_pixel_scales': True,
                                if cfg.backbone.use_pixel_scales:
                                    w = scale * ar / cfg.max_size #yolact++ max size 550
...
class FPN(ScriptModuleWrapper):
    ...
    def __init__(self, in_channels):
        ...
        # yolact++ use_conv_downsample True ??
        # yolact++ num downsample 2
    ...
    def forward(self, convouts:List[torch.Tensor]):
        ...
        # lat_layers  in_channels out_channels
        #     0           512x4       256
        #     1           256x4       256
        #     2           128x4       256
        for lat_layer in self.lat_layers:
            ...
            if j < len(convouts) - 1:
                ...
                ####                                        interpolation mode always bilinear ??
                x = F.interpolate(x, size=(h, w), mode=self.interpolation_mode, align_corners=False)
            
            x = x + lat_layer(convouts[j])
            out[j] = x

        # out_idx | source_channel_size | out_channel_size
        #     0            128x4                  256
        #     1            256x4                  256
        #     2            512x4                  256
        ...
        # yolac relu_downsample_layers False ?
...
####_____________________________________________________________________________________________________________________
    ...
    def forward(self, x) :
        ...
        proto_out = None
                                                # eval_mask_branch almost always True , except in detection only mode
        if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
            with timer.env('proto'):
                # yolact use index 0 as proto_src
                proto_x = x if self.proto_src is None else outs[self.proto_src]

                # yolact self.num_grids = 0
                if self.num_grids > 0:
        ...
        with timer.env('pred_heads'):
            ...
            # yolact selected_layers = 0,1,2,3,4
            for idx, pred_layer in zip(self.selected_layers, self.prediction_layers):
                pred_x = outs[idx]

                ##______________________________________________________________________________________________________
                # mask_proto_prototypes_as_features always false
                ...
                # yolact share_prediction_module True
        ...
        #### _______________________ NOT TRAINING OUTPUT  _______________________________
        else:
            if cfg.use_mask_scoring:
