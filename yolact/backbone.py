...
class ResNetBackbone(nn.Module):
    ...
    # from add_layer    ## Bottleneck , 256 , blocks = 1 , stride = 2
    def _make_layer(self, block , planes, blocks, stride=1, dcn_layers=0, dcn_interval=1):
        ...
        layers.append(
            # Bottleneck
            # inplanes, planes, stride = 1, downsample = None, norm_layer = nn.BatchNorm2d, dilation = 1, use_dcn = False
            block(self.inplanes, planes, stride, downsample, self.norm_layer, self.dilation, use_dcn=use_dcn)
        )
        ...
        for i in range(1, blocks):
            ...
            layers.append(
                # inplanes, planes, stride = 1, downsample = None, norm_layer = nn.BatchNorm2d, dilation = 1, use_dcn = False
                block(self.inplanes, planes, norm_layer=self.norm_layer, use_dcn=use_dcn)
            )
    ...
    # for yolact++ 101 this add_layer is not used ??
    def add_layer(self, conv_channels=1024, downsample=2, depth=1, block=Bottleneck):
        """ Add a downsample layer to the backbone as per what SSD does. """
                        ## Bottleneck , 256 , blocks = 1 , stride = 2
...
def construct_backbone(cfg):
    ...
    # resnet 101 yolact++ selected layers = 1,2,3
