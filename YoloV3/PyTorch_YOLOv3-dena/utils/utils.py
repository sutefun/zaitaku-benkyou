...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        auau = image_pred[:, 5:5 + num_classes]
        class_pred = torch.max( auau , 1 )
        class_pred = class_pred[0]
        fdf = image_pred[:, 4]
        non_thre = image_pred[:, 4] * class_pred

        conf_mask = ( non_thre >= conf_thre).squeeze()
        print( conf_mask.max() )
        ad= conf_mask.numpy()
        ad_idx = np.argwhere( ad == 1 )
        old_image_pred = image_pred
        ...
        u1 = image_pred[:, 4]
        u2 = image_pred[:, 4][:, None]
        u3 = image_pred[:, 5:] * u2
        u4 = ( u3 >= conf_thre)
        u44 = u4.numpy()
        ind = u4.nonzero()
        # Detections ordered as (x1, y1, x2, y2, obj_conf, class_conf, class_pred)
        det1 = image_pred[ind[:, 0], :5]
        det2 = image_pred[ind[:, 0], 5 + ind[:, 1]]
        det22 = det2.unsqueeze(1)
        det3 = ind[:, 1].float()
...
def preprocess(img, imgsize, jitter = 0 , random_placing=False):
    """
